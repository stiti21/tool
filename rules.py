...
# coding: utf-8
"""
Email CSV Phishing Detection - Full Version with IP, Risk Counters, and Colors
...
class PhishingDetector:
    def __init__(self):
        ...
        # Suspicious words often used in phishing emails
        suspicious_words = [
        ...
        # Compile regex patterns for suspicious words
        self.SUSPICIOUS_PATTERNS = [
            re.compile(r"\b" + re.escape(w) + r"\b", re.IGNORECASE)
            for w in suspicious_words
        ]

        # DNS resolver for checking MX records
        self.resolver = dns.resolver.Resolver()
        self.resolver.lifetime = 5.0
        self.resolver.timeout = 3.0
        self.mx_cache = {}

    # Rule 1: Compare From and Return-Path emails
    ...
    def rule3_domain(self, from_h):
        ...
        try:
            ...
            if any(part in self.SUSPICIOUS_TLDS for part in tld_parts): score +=25
        except Exception: pass
        if not self._has_mx_records(domain): score +=25
        brand_imp=None
        risk="High" if score>=60 else "Medium" if score>=30 else "Low"
        return {"Rule3_Risk_Score":score,"Rule3_Risk_Level":risk,"Rule3_Domain":domain,"Rule3_Brand_Impersonation":brand_imp}

    # Helper: Check if domain has MX records
    ...
    def rule5_content(self, body_text, body_html):
        text=(body_text or "")[:200000]; html=(body_html or "")[:200000]
        URL_RE = re.compile(r"https?://[^\s\"\'<>]+", re.IGNORECASE)
        def contains_suspicious(txt):
            for patt in self.SUSPICIOUS_PATTERNS:
                if patt.search(txt or ""): return True
            return False
        def extract_links_from_html(h):
            ...
            try:
                ...
                for img in soup.find_all("img"):
                    ...
                    if href: links.append(("[IMAGE]", href))
            except Exception: pass
            return links
        def extract_links_from_text(t):
            try: return [(u,u) for u in URL_RE.findall(t or "")]
            except Exception: return []
        def check_mismatch(visible, actual):
            try:
                v=(visible or "").strip(); a=(actual or "").strip()
                if not a or v=="[IMAGE]": return False
                if re.match(r"^https?://", v, re.IGNORECASE): return v.lower()!=a.lower()
                try:
                    actual_domain=tldextract.extract(a).registered_domain or ""
                    visible_domain=tldextract.extract(v).registered_domain or ""
                    if actual_domain and visible_domain: return actual_domain.lower()!=visible_domain.lower()
                except Exception: pass
        ...
        reasons=[]
        if suspicious_found: reasons.append("suspicious_words")
        if link_mismatch: reasons.append("link_mismatch")
        return {
